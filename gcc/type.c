...
// Make a name local to the current compilation unit by prepending it with a
// "compilation unit ID" ("CU" and a random number) followed by '#' and the
...
static void
MakeNameCULocal(tree decl, struct XIL_CString* name) {
    ...
    sprintf((char*) full_name.str, "CU%08x#%s:%s", compilation_unit_id, file, name->str);
